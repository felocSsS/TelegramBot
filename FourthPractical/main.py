...
async def cmd_start(message: types.Message):
    ...
    await message.reply("Приветствую Сталкер! Как тебя кличут в зоне")
...
async def process_name(message: types.Message, state: FSMContext):
    ...
    await Form.next()
    await message.reply("В какой группировке состоишь?")
...
async def process_age(message: types.Message, state: FSMContext):
    ...
    await message.reply("В каком году ты пришел в зону", reply_markup=markup)

@dp.message_handler(lambda message: message.text not in ["2017", "2018", "2019", "2020"], state=Form.startedu)
async def process_gender_invalid(message: types.Message):
    return await message.reply("В эти года зоны не было. Выбери правильный вариант")
